...
**版本：v1.1 防疲劳版**
**日期：2026-05-07**
**状态：v2 设计稿等效 Figma，供开发落地**

---

## 一、设计理念

**矿业大脑 + 舒适暗色 + Raycast 工具感**

- 深色背景，但不过深（#0d1520）→ 保持沉浸感，同时不伤眼
- 毛玻璃半透明卡片 → 层次分明、轻盈不闷
- 蓝色作为主强调色 → 视觉统一
- 行高充分（1.6）→ 文字有呼吸感，不疲劳
- 阴影用 rgba(0,0,0,0.3) → 卡片有"悬浮感"
- 功能贴合矿业大脑：智能调度、Agent 协同、实时状态

**⚠️ 防疲劳原则**
- 最深背景不用纯黑（#000），用深蓝灰（#0d1520）
- 文字对比度充分（正文 #cbd5e1，不低于 #94a3b8）
- 卡片背景比页面背景只深一个层级，不跳脱
- 避免大面积高饱和色块，用点缀而非铺满
...
| 层级 | 色值 | 用途 |
|------|------|------|
| bgRoot | `#0d1520` | 页面最底层背景（深蓝灰，非纯黑，防疲劳） |
| bgSurface | `#111827` | 页面背景（比 root 略浅） |
| bgCard | `rgba(17,30,50,0.78)` | 毛玻璃卡片背景 |
| bgElevated | `#162035` | 抬升元素（TabBar 等） |
| bgOverlay | `rgba(13,21,32,0.92)` | 弹窗/遮罩层 |
| bgGlass | `rgba(17,30,50,0.70)` | 毛玻璃效果 |
...
| 类型 | 色值 | 用途 |
|------|------|------|
| borderSubtle | `rgba(255,255,255,0.07)` | 卡片默认边框 |
| borderDefault | `rgba(255,255,255,0.12)` | 分割线/输入框边框 |
| borderActive | `rgba(56,189,248,0.55)` | 聚焦/激活态边框 |
...
| primaryGlow | `rgba(56,189,248,0.20)` | 光晕背景（稍减淡） |
...
| 类型 | 色值 | 用途 |
|------|------|------|
| textPrimary | `#e2e8f0` | 标题/重要文字（提亮，不刺眼） |
| textSecondary | `#cbd5e1` | 正文（充分对比度） |
| textMuted | `#64748b` | 辅助/次要文字 |
...
export const C = {
  // 背景层级 — 防疲劳版
  bgRoot:          '#0d1520',   // 深蓝灰，非纯黑
  bgSurface:       '#111827',   // 略浅于 root
  bgCard:          'rgba(17,30,50,0.78)',
  bgElevated:      '#162035',
  bgOverlay:       'rgba(13,21,32,0.92)',
  bgGlass:         'rgba(17,30,50,0.70)',
  ...
  // 文字色 — 防疲劳，提高对比度
  textPrimary:      '#e2e8f0',   // 提亮，不刺眼
  textSecondary:    '#cbd5e1',   // 充分对比
  textMuted:        '#64748b',
  textDisabled:     '#1e293b',
